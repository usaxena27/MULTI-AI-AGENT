...
# IMPORTANT: read backend address from env variable.
API_BASE = os.getenv("API_URL", "http://127.0.0.1:9999")
API_URL = API_BASE.rstrip("/") + "/chat"
st.info(f"Backend API: `{API_URL}`")  # shows the URL used (remove in prod)
...
def call_backend(api_url: str, payload: dict):
    ...
    try:
        ...
        logger.exception("Could not connect to backend.")
        raise CustomException(f"Could not connect to backend at {api_url}.", e)
    except requests.exceptions.Timeout as e:
        logger.exception("Request to backend timed out.")
        raise CustomException("Request to backend timed out. Please try again.", e)
    except requests.exceptions.HTTPError as e:
        backend_body = e.response.text if e.response is not None else "No response body."
        logger.exception("Backend returned HTTP error.")
        raise CustomException(f"Backend returned an error: {e}. Details: {backend_body}", e)
    except Exception as e:
        logger.exception("Unexpected error while sending request to API.")
        raise CustomException("Failed to communicate to backend due to an unexpected error.", e)
...
if st.button("Ask Agent") and user_query.strip():
    ...
    try:
        ...
        try:
            ...
            agent_response = data.get("response", "")
            if not agent_response:
                st.warning("Backend returned no 'response' field.")
            else:
                st.subheader("Agent Response")
